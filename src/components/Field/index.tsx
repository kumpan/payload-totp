    ...
    <div className={styles.root}>
      ...
      <div className={styles.text}>
        <label className="field-label">
          ...
          {configured && <span className={styles.status}>{i18n.t('totpPlugin:configured')}</span>}
        </label>
        <span className={styles.description}>{i18n.t('totpPlugin:fieldDescription')}</span>
      </div>
      <div className={styles.action}>
        {configured && !pluginOptions.forceSetup && (
          <Remove i18n={i18n} user={user} payload={payload} pluginOptions={pluginOptions} />
        )}
        {!configured && !pluginOptions.forceSetup && <Setup i18n={i18n} backUrl={url} />}
      </div>
    </div>
  )
}
